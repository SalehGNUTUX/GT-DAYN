const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["assets/ElectronAdapter-Box8xpdD.js","assets/DbAdapter-BfKqjr-p.js","assets/CapacitorAdapter-CzTGqiLv.js","assets/WebSQLiteAdapter-Bfog9H_J.js"])))=>i.map(i=>d[i]);
(function polyfill() {
...
class DebtService {
  ...
  async addDebt({ personId, type, title, amount, note = null }) {
    const { lastInsertRowid } = await this._db.run(`
      INSERT INTO debts (person_id, type, title, amount, remaining, note)
      VALUES (?, ?, ?, ?, ?, ?)
    `, [personId, type, title.trim(), amount, amount, note]);
    return lastInsertRowid;
  }
  async updateDebt(id, fields) {
    const allowed = ["title", "amount", "remaining", "note", "status", "sort_order", "type"];
...
class BudgetService {
  ...
  async getSuggestion(month = BudgetService.currentMonth()) {
    ...
    const debts = await this._db.query(`
      ...
      LIMIT 1
    `);
    if (!debts.length) return { type: "save", amount: summary.remaining };
    return {
      type: "pay_debt",
      amount: Math.min(summary.remaining, debts[0].remaining),
      debt: debts[0],
...
async function resolveAdapter() {
  ...
  if ((_b = (_a = window.Capacitor) == null ? void 0 : _a.isNativePlatform) == null ? void 0 : _b.call(_a)) {
    const { CapacitorAdapter } = await __vitePreload(async () => {
      const { CapacitorAdapter: CapacitorAdapter2 } = await import("./CapacitorAdapter-CzTGqiLv.js");
  ...
  const { WebSQLiteAdapter } = await __vitePreload(async () => {
    const { WebSQLiteAdapter: WebSQLiteAdapter2 } = await import("./WebSQLiteAdapter-Bfog9H_J.js");
...
function fmt(n, currency2 = "") {
  const s = Math.round(n).toLocaleString("en-US");
  return currency2 ? `${s} ${currency2}` : s;
}
...
window.addEventListener("gt-dayn:ready", async () => {
  ...
  try {
    ...
    if (savedTheme) {
      ...
      await app.db.run(`UPDATE settings SET value=? WHERE key='theme'`, [prefersDark ? "dark" : "light"]);
    }
    if (savedCurrency) {
  ...
  debts.forEach((d, i) => {
    ...
    card.innerHTML = `
        <div class="card-top">
          ...
          <div class="card-amount ${done ? "done" : ""}">${done ? "✓" : fmt(d.remaining, currency)}</div>
        </div>
        <div class="progress-lbl">
          <span class="pct">${pct}% مسدّد</span>
          <span class="orig">من ${fmt(d.amount, currency)}</span>
        </div>
        ...
        <div class="history-panel" id="hist-${d.id}">
          <div class="amounts-row">
            <div class="amount-cell">
              ...
              <div class="ac-val">${fmt(d.amount, currency)}</div>
            </div>
            <div class="amount-cell ac-blue">
              <div class="ac-lbl">المدفوع</div>
              <div class="ac-val">${fmt(paid, currency)}</div>
            </div>
            <div class="amount-cell ${done ? "ac-green" : ""}">
              <div class="ac-lbl">المتبقي</div>
              <div class="ac-val">${fmt(d.remaining, currency)}</div>
            </div>
          </div>
  ...
  container.innerHTML = dues.map((d) => `
      <div class="due-notif" onclick="openPayModalForScheduled(${d.debt_id},${d.id},${d.amount})">
        <div class="due-notif-info">
          ...
          <span class="due-notif-amt">دفعة مستحقة: ${fmt(d.amount, currency)}</span>
        </div>
...
window.saveDebt = async function() {
  ...
  if (editDebtId) {
    await app.debts.updateDebt(editDebtId, {
      title,
      amount,
      note: note || null,
      type: debtType
    ...
    await app.debts.addDebt({ personId, type: debtType, title, amount, note: note || null });
  }
  closeAllModals();
    ...
    sugEl.innerHTML = `
        <div class="suggestion-card">
          ...
          <div>
            ...
            <div class="sug-body">اقتراح: سدّد ${fmt(sug.amount, currency)} على "${sug.debt.title}" — ${sug.debt.person_name}</div>
          </div>
        </div>`;
  } else {
    sugEl.innerHTML = "";
...
window.promptIncome = async function() {
  const bm = await app.budget.getOrCreateMonth(currentMonth);
  const val = prompt("أدخل الدخل / الميزانية لهذا الشهر:", bm.income || "");
  if (val !== null && !isNaN(+val) && +val >= 0) {
    await app.budget.setIncome(currentMonth, +val);
    await renderBudget();
  }
};
...
function _updatePwdUI() {
  ...
  document.getElementById("pwd-status").textContent = active ? "مفعّل ✓" : "غير مفعّل";
  ...
  if (changeRow) changeRow.style.display = active ? "" : "none";
}
...
(function() {
  if (localStorage.getItem(_PWD_KEY)) {
...
window.openCalc = function(targetInputId) {
  var _a;
...
const _origSaveDebt = window.saveDebt;
window.saveDebt = async function() {
  if (editDebtId) {
    const old = await app.debts.getDebt(editDebtId);
    await _origSaveDebt();
    const updated = await app.debts.getDebt(editDebtId);
    if (old && updated) {
      if (old.amount !== updated.amount)
        await recordDebtEdit(editDebtId, "amount", old.amount, updated.amount);
      if (old.type !== updated.type)
        await recordDebtEdit(
          editDebtId,
          "type",
          old.type === "receivable" ? "لي عند" : "عليّ",
          updated.type === "receivable" ? "لي عند" : "عليّ"
        );
      if (old.title !== updated.title)
        await recordDebtEdit(editDebtId, "title", old.title, updated.title);
    }
  } else {
    await _origSaveDebt();
  }
