...
class CapacitorAdapter extends DbAdapter {
  constructor() {
    super();
    this._db = null;
    this._SQL = null;
  ...
  _migrate() {
    const add = (t, c, d) => {
      try {
        this._db.run(`ALTER TABLE ${t} ADD COLUMN ${c} ${d}`);
      } catch {
      }
    };
...
INSERT OR IGNORE INTO settings VALUES ('currency',     'MAD');
INSERT OR IGNORE INTO settings VALUES ('currency_sym', 'د.م');
...
CREATE TABLE IF NOT EXISTS debts (
  ...
  currency    TEXT    DEFAULT 'MAD',           -- عملة الدين
  note        TEXT,
